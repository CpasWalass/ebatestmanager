        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition p-5 flex flex-col gap-4 {{ !$member->is_active ? 'opacity-60 grayscale' : '' }}">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button wire:click="resendCredentials({{ $member->id }})" wire:confirm="Régénérer et renvoyer les identifiants de {{ $member->name }} ? Son ancien mot de passe sera invalidé." class="text-gray-400 hover:text-blue-500 transition" title="Renvoyer les identifiants par email">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                    </button>
                    <button wire:click="deleteUser({{ $member->id }})" wire:confirm="Supprimer définitivement le compte de {{ $member->name }} ?" class="text-gray-400 hover:text-red-600 transition" title="Supprimer définitivement">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <form wire:submit="save" class="px-6 py-5 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="email" wire:model="email" class="w-full rounded-xl border bg-white dark:bg-gray-700 px-4 py-2 text-sm focus:outline-none focus:ring-2 {{ $errors->has('email') ? 'border-red-500 dark:border-red-500 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-[#8b0000]' }}">
                        @error('email')
                            <p class="mt-1.5 text-red-600 dark:text-red-400 text-xs font-semibold flex items-start gap-1.5">
                                <svg class="w-3.5 h-3.5 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                                <span>{{ $message }}</span>
                            </p>
                        @enderror
                    </div>
