...
<style>
    /* ... autres styles globaux si nécessaires ... */
</style>
...
<div class="min-h-screen flex flex-col" style="padding-bottom: 90px;">
        ...
        <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 h-full flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3 min-w-0">
                <div class="flex items-center justify-center h-10 w-10 sm:w-auto">
                    <img src="{{ asset('images/logo-dark.png') }}" alt="Logo" class="h-10 w-auto object-contain" onerror="this.outerHTML='<span class=\'text-[#8b0000] font-bold text-sm tracking-tight\'>EB</span>'">
                </div>
    ...
    <div class="dock pointer-events-auto z-50">
            ...
            @php
                ...
                if ($isChef) {
                    ...
                    // Journal — visible par Chef uniquement
                    $dockItems[] = [
                        'id'     => 'dock-journal',
                        'label'  => 'Journal',
                        'route'  => 'journal.global',
            ...
            @foreach($dockItems as $item)
                <a href="{{ route($item['route']) }}"
                    ...
                    {{-- Badge --}}
                    @if($item['badge'] > 0)
                        <span class="absolute -top-1 right-0 w-4 h-4 text-white text-xs rounded-full flex items-center justify-center font-bold z-20 shadow-sm"
...
<style>
    ...
    .dock {
        ...
        background: #1f2b3d;
        clip-path: path(
            "M35 70 L35 38 C85 8 170 0 210 0 C250 0 335 8 385 38 L385 70 Z"
        );
